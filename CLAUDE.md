...
Every file in `tests/data/` is a real registry response and owes an entry in
`tests/data/PROVENANCE.toml`; `uv run pytest -m network` is the only thing that
verifies one. The procedure, the fields an entry carries, and the list of what
the networked comparison ignores are in the `registry-fixtures` skill
(`.claude/skills/registry-fixtures/SKILL.md`), which two tests in
`tests/test_fixture_provenance.py` read.
